...
export default function DbListCsv({id,data}:{id:string,data:NodeData}){
  ...
  async function run(input:Res):Promise<Res>{
    try {
      const keyToUseList = config.keyToMemoryCsv; // 使用与 DbRead 相同的键
      const lines = await lineStorage.listLines(keyToUseList);
      let csvStr = "";
      
      if (lines.length > 0) {
        csvStr = JSON.stringify(lines);
      } else {
        csvStr = "记忆为空";
      }
      
      return updateResData(input,{success: true,msg: csvStr,})
    } catch (error) {
      return updateResData(input,{success: false,msg: `记忆列表失败: ${error}`})
    }
  }

  async function callbackTrigger(){
    if(v0==="1"&&(relations[id]!=undefined&&relations[id].length>0)){
      await lineStorage.clearLines(config.keyToMemoryCsv);
    }
  }
          ...
          </Tooltip>
          {
            v0==="0"&&(
              <Button danger type="primary" onClick={()=>{
                lineStorage.clearLines(config.keyToMemoryCsv);
                setDebug({
                  show: true,
                  data: `已清空记忆列表中的所有数据`,
                  loading: false,
